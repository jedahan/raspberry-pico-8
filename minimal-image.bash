...
WORKDIR=/home/pi
piho run "$PIHO_IMAGE_NAME" cd "$WORKDIR" && tar xf "$SDL2_SRC" && mkdir "${SDL2_SRC%%.tar.gz}/build"
# build with OpenGL ES support only (fullscreen)
piho run "$PIHO_IMAGE_NAME" cd "$WORKDIR"/"${SDL2_SRC%%.tar.gz}"/build && ../configure --host=armv7l-raspberry-linux-gnueabihf --disable-pulseaudio --disable-esd --disable-video-mir --disable-video-wayland --disable-video-x11 --disable-video-opengl && make -j4 && make install
...
curl -OL https://www.libsdl.org/release/"$SDL2_SRC" && piho copy "$SDL2_SRC" /home/pi
