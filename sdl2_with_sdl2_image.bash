...
piho run "$PIHO_IMAGE_NAME" bash -c "cd $WORKDIR && tar xf $SDL2_IMAGE_SRC && mkdir ${SDL2_IMAGE_SRC%%.tar.gz}/build"
piho run "$PIHO_IMAGE_NAME" bash -c "cd ${WORKDIR}/${SDL2_IMAGE_SRC%%.tar.gz}/build && ../configure && make -j4 && make install"
piho run "$PIHO_IMAGE_NAME" bash -c "rm -rf ${WORKDIR}/${SDL2_IMAGE_SRC%%.tar.gz}"
