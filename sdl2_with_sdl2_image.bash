...
piho run $PIHO_IMAGE_NAME apt-get remove -y --purge scratch pypy-upstream sonic-pi freepats libraspberrypi-doc oracle-java8-jdk wolfram-engine
piho run $PIHO_IMAGE_NAME apt-get autoremove -y
...
piho run $PIHO_IMAGE_NAME bash -c "cd ${WORKDIR} && tar xf $SDL2_IMAGE_SRC && cd ${SDL2_IMAGE_SRC%%.tar.gz} && mkdir -p build && cd build && ../configure && make -j4 && make install"
